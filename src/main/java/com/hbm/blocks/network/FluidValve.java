...
public class FluidValve extends FluidDuctBase implements IDynamicModels, ILookOverlay {

    public static final PropertyInteger META = PropertyInteger.create("meta", 0, 1);

    public FluidValve(Material mat, String name) {
        super(mat);
    ...
    public void registerSprite(TextureMap map) {
        map.registerSprite(new ResourceLocation(Tags.MODID, "blocks/fluid_valve_on"));
        map.registerSprite(new ResourceLocation(Tags.MODID, "blocks/fluid_valve_off"));
    }
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand, EnumFacing side, float hitX, float hitY, float hitZ) {
        ...
        if(!player.isSneaking()) {
            ...
            TileEntityFluidValve te = (TileEntityFluidValve) world.getTileEntity(pos);

            if(meta == 0) {
                world.setBlockState(pos, state.withProperty(META, 1), 2);
                world.playSound(null, pos, HBMSoundHandler.reactorStart, SoundCategory.BLOCKS, 1.0F, 1.0F);
            } else {
                world.setBlockState(pos, state.withProperty(META, 0), 2);
    ...
    public void bakeModel(ModelBakeEvent event) {
        try {
            ...
            textureMap.put("all", new ResourceLocation(Tags.MODID, "blocks/fluid_valve_off").toString());
            textureMap.put("particle", new ResourceLocation(Tags.MODID, "blocks/fluid_valve_off").toString());
            ...
            textureMap.put("all", new ResourceLocation(Tags.MODID, "blocks/fluid_valve_on").toString());
            textureMap.put("particle", new ResourceLocation(Tags.MODID, "blocks/fluid_valve_on").toString());
